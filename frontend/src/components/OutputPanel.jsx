...
function FinalView({ finalState }) {
  ...
  const appUrls = finalState.appUrls || {};
  const frontendUrl = finalState.frontendUrl || appUrls.frontendUrl || "";

  return (
    <div className="output-section">
      {frontendUrl ? (
        <div className="link-grid">
          <a className="result-link" href={frontendUrl} target="_blank" rel="noreferrer">
            <span className="stat-label">APP URL</span>
            <span className="result-link-value">{frontendUrl}</span>
          </a>
        </div>
      ) : (
        <Empty text="App URL appears after launch" />
      )}
    </div>
  );
}
